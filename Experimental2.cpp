...
void Chirp::scroll_my_posts(post* n, int placement_given)
{
  ...
  else
  {
    ...
    else if (response == 'd')
    {
      ...
      if (current->next == nullptr)
      {
        post* n = delete_chirp(current);
        if (n == NULL)
        {
          return;
        }
        placement--;
        scroll_my_posts(n, placement);
      }
...
post* Chirp::delete_chirp(post* n)
{
  post* temporary = n;
  if (temporary->next == nullptr)
  {
    post* secondary_temp = temporary -> previous;
    delete temporary;
    secondary_temp -> next = nullptr;
    return secondary_temp;
  }

  else if (temporary -> previous == nullptr)
  {
    post* second_temp = temporary;
    temporary = second_temp -> next;
    temporary -> previous = nullptr;
    head = temporary;
    delete second_temp;
    return temporary;
  }
  ...
  {
    post* third_temp = n;
    post* node_before = third_temp -> previous;
    node_before -> next = third_temp -> next;
    node_before -> previous = third_temp -> previous -> previous;
    delete n;
    //delete third_temp;
    return node_before;
