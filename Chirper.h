...
// The hang_man() function is an accessory function that plays a minigame. Precondition: None. Postcondition: Can return a string to add_chirp_post(), otherwise, returns to menu.
void Chirp::hang_man()
{
  Clear();
  int tries = 0;
  ...
  while (alive)
  {
    ...
    if (in_word == false)
    {
      ...
      else if (tries == 5)
      {
        ...
        cout << "-----------------" << endl;
        cout << "Tries: " << tries << endl;
        cout << "No more moves. The hangman is hung and the word was " << temp << ". How do you feel?" << endl;
        alive = false;
      }
    }
    continue;
  ...
  if (!alive)
  {
    cout << "Would you like to share on chirper? Type 'y' for yes and type 'n' for no." << endl << "Response: ";
    char input;
...
// The clear() function is an accessory function that is used in the hang_man() function to clear the screan. Precondition: None. Postcondition: None.
void Chirp::Clear()
{
  for (int n = 0; n < 10; n++)
  {
...
void Chirp::scroll_my_posts(post* n, int placement_given)
{
  ...
  else
  {
    ...
    // If the response is d(elete), delete the node at the current node.
    else if (response == 'd')
    {
      cout << "Deleting post. " << endl;
      // if the next node to the current node is NULL and the previous node is also NULL. Then this is the last node to delete.
      ...
      // if the next node to the current node is NULL, then this is the tail of the linked list. Decrease the value of placement so that the chirp's are numbered correctly.
      if (current->next == nullptr)
      {
      ...
      // if the previous node to the current node is NULL, then this is the head of the linked list.
      ...
      // Otherwise, if neither next nore previous are null, then this is in the middle of the linked list. Decrease the value of placement so that the chirp's are numbered correctly.
      else
      {
        delete_chirp(current, current -> previous);
        placement--;
    ...
    // If the user responds with 'r', state to the user that you cannot rechirp your own tweets.
    else if (response == 'r')
    {
    ...
    // If the user responds with 'x', return to the main menu;
    else if (response == 'x')
    {
...
void Chirp::scroll_thru_friend_posts(post* n, int placement_given)
{
  ...
  else
  {
    ...
    // If the response is d(elete), let the user know that they cant delete another users posts.
    else if (response == 'd')
    {
    ...
    // If the resposne is r(echirp), rechirp the current node.
    else if (response == 'r')
    {
      // if current -> reChirp is true,the node has already been reChirped and therefore, you can't rechirp it again.
...
// The add_chirp_post() function is used to delete nodes in the linked list (of chirps) depending on where the perameter "to_delete" is in the linked list. Precondition: function takes in to_delete (The node at which we are deleting) and to_attach (The node before/after to_delete, that we will attach to the next/previous node of to_delete, if needed.). Postcondition: the node to_delete will be deleted, and to_attach will point to the next/previous node correctly.
void Chirp::delete_chirp(post* to_delete, post* to_attach)
{
  // if the next node to the to_delete node is null, then we are at the tail.
  if (to_delete->next == nullptr)
  {
    // make to_attach the last node in the linked list
    to_attach -> next = nullptr;
    delete to_delete;
  }

  // If the previous node to the to_delete node is null, then we are at the head. the node previous to to_attach will be null, and it will be made the new head.
  ...
  // If the next and previous nodes are not null, then create a pointer called temp which will point to the next, next node to to_attach. the next node to to_attach will skip to_delete and point to the node after it. the previous node to temp will be to_attach.
  else
  {
...
// if there was only one node left in the linked list, this function will delete it.
...
void Chirp::menu()
{
  ...
  while (base == true)
  {
    ...
    // If the user entered 6, turn the boolean "base" false which will end the end the menu() function
    if (option == 6)
    {
      base = false;
    }
    ...
    // If the user entered 4, the process of playing hangman will commense.
    else if (option == 5)
    {
      hang_man();
    }

    // If the user entered anything not 1,2,3,4,5 or 6, display a warning and repeat the menu options.
    else
    {
